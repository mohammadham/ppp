...
\midrule
\lr{Frequency (Monobit) Test} & 0.0157 & پذیرفته‌شده (PASSED) \\
\lr{Block Frequency Test ($m=128$)} & 0.6214 & پذیرفته‌شده (PASSED) \\
\lr{Cumulative Sums (Forward)} & 0.0231 & پذیرفته‌شده (PASSED) \\
\lr{Runs Test} & 0.5726 & پذیرفته‌شده (PASSED) \\
\lr{Longest Run of Ones Test} & 0.3129 & پذیرفته‌شده (PASSED) \\
\lr{Binary Matrix Rank Test} & 0.4891 & پذیرفته‌شده (PASSED) \\
\lr{Discrete Fourier Transform (Spectral) Test} & 0.5124 & پذیرفته‌شده (PASSED) \\
\lr{Non-overlapping Template Matchings} & 0.4782 & پذیرفته‌شده (PASSED) \\
\lr{Overlapping Template Matchings} & 0.3654 & پذیرفته‌شده (PASSED) \\
\lr{Maurer's Universal Statistical Test} & 0.6912 & پذیرفته‌شده (PASSED) \\
\lr{Approximate Entropy Test} & 0.4855 & پذیرفته‌شده (PASSED) \\
\lr{Random Excursions Test} & 0.5420 & پذیرفته‌شده (PASSED) \\
\lr{Random Excursions Variant Test} & 0.4190 & پذیرفته‌شده (PASSED) \\
\lr{Serial Test ($m=16$)} & 0.7321 & پذیرفته‌شده (PASSED) \\
\lr{Linear Complexity Test} & 0.6015 & پذیرفته‌شده (PASSED) \\
\bottomrule
\end{tabular}
\end{table}
