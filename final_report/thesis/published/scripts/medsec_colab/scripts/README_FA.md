...
## اصلاح MIXED (نسخه فعلی)
برای CHASE_DB1+STARE از `MIXED.ipynb` استفاده کنید؛ راهنمای علت‌ها، مقایسه دو اجرای قدیمی و محدودیت‌های باقی‌مانده در `../mixed_diagnosis/REPORT_FA.md` است. manifest بیمارمحور، وزن/خروجی نسخه‌دار و گزارش‌های تفکیک‌شده جای پچ‌های زمان اجرا را گرفته‌اند. `mixed_diagnostic.json` فقط تشخیص دستگاه موجود در پچ قبلی است؛ پایداری، انتساب مقاله یا ابرآشوب آن تأیید نشده است. `appendix_experiment.json` دوباره مربوط به دستگاه واقعی پیوست است. مقدارهای معادلهٔ ۳–۲ هنوز باید از منبع معتبر تکمیل شوند. هیچ run یا وزن قدیمی برای ادامه روی تقسیم جدید استفاده نشود.
