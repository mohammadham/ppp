...
\textbf{فرضیه اول ($H_1$):} پویایی کلید و مقاومت تفاضلی & $\mathrm{NPCR} > 99.5693\%$ & $\mathrm{NPCR} = 99.6069\% \pm 0.0103\%$ & \textbf{تایید قطعی \lr{(PASSED)}} \\
 & $\mathrm{UACI} \in [33.31\%,\allowbreak{} 33.61\%]$ & $\mathrm{UACI} = 33.4571\% \pm 0.0413\%$ &  \\
\textbf{فرضیه دوم ($H_2$):} تصادفی‌سازی و آنتروپی آرمانی & $H(m) \approx 8.00\text{ bits}$ & $H(m) = 7.999427\text{ bits}$ & \textbf{تایید قطعی \lr{(PASSED)}} \\
 & $r_{xy} \approx 0.0000$ & $r_{xy} = +0.0010$ &  \\
 & $\text{NIST SP800-22 } p \ge 0.01$ & پاس کردن ۱۵ آزمون ($p \ge 0.0125$) &  \\
\textbf{فرضیه سوم ($H_3$):} شفافیت استگو و صیانت از \lr{ROI} & $\mathrm{PSNR}_{\mathrm{Total}} > 50\text{ dB}$ & $\mathrm{PSNR}_{\mathrm{Total}} = 53.54\text{ dB}$ & \textbf{تایید قطعی \lr{(PASSED)}} \\
 & $\mathrm{PSNR}_{\mathrm{ROI}} = \infty$ & $\mathrm{PSNR}_{\mathrm{ROI}} = \infty$ (عدم نویز) &  \\
 & $\mathrm{SSIM} \ge 0.9990$ & $\mathrm{SSIM} = 0.9999$ &  \\
\bottomrule
...
مطابق فرضیه اول تدوین‌شده در بخش ۱{-}۴، ادعا گردید که استخراج ویژگی‌های آماری ناحیه تشخیصی \lr{ROI} و تولید کلید هَش پویا، موجب ایجاد حساسیت شدیدی نسبت به کوچک‌ترین تغییر در تصویر ورودی گشته و مقاومت تفاضلی سیستم را تضمین می‌نماید. نتایج تجربی فصل چهارم (بخش ۴{-}۶) اثبات نمود که با تغییر تنها ۱ پیکسل در تصویر ورودی، کلید هَش ۲۵۶ بیتی $K_{\mathrm{hash}}$ به صورت کامل دگرگون شده و متغیرهای حالت اولیه سیستم ۵بعدی ابرآشوبی را کاملاً تغییر می‌دهد \ThesisCite{1}, \ThesisCite{2}, \ThesisCite{6}. شبیه‌سازی‌های انجام‌شده بر روی ۱۰۰ جفت تصویر متناظر نشان داد که متوسط شاخص \lr{NPCR} به \textbf{۹۹.۶۰۶۹\SourceGlyph{066A} \ensuremath{\pm} ۰.۰۱۰۳\SourceGlyph{066A}} (فراتر از حد آستانه تئوریک ۹۹.۵۶۹۳\SourceGlyph{066A}) و متوسط شاخص \lr{UACI} به \textbf{۳۳.۴۵۷۱\SourceGlyph{066A} \ensuremath{\pm} ۰.۰۴۱۳\SourceGlyph{066A}} (در بازه آرمانی $[33.3115\%,\allowbreak{} 33.6156\%]$) رسیده و نرخ قبولی آزمون‌های تفاضلی بر روی تمامی ۱۰۰ جفت تصویر برابر با \textbf{۱۰۰\SourceGlyph{066A}} گردیده است. این نتایج، خنثی‌سازی کامل حملات متن‌واضح برگزیده \lr{(CPA)}، متن‌واضح معلوم \lr{(KPA)} و حملات تفاضلی را اثبات نموده و فرضیه اول را به صورت قطعی تایید می‌نماید \ThesisCite{1}, \ThesisCite{4}. همچنین، به کارگیری پرونده کمکی احرازهویت‌شده \lr{(Recovery Sidecar)} \texttt{recovery.msr} مبتنی بر استاندارد \lr{AES-GCM-256} تضمین نمود که بازگشایی قطعی و بیت‌به‌بیت تصویر اصلی در سمت گیرنده بدون هیچ‌گونه اتلاف اطلاعات بالینی ($\text{PSNR}_{\mathrm{ROI}} = \infty$) میسر می‌سازد \ThesisCite{2}.\par
...
مطابق فرضیه سوم، ادعا شد که ماژول پنهان‌نگاری آگاه به برجستگی بصری \lr{(Saliency{-}aware)} فراداده بیمار را بدون ایجاد کوچک‌ترین نویز در ناحیه \lr{ROI} جای‌دهی کرده و شفافیت استگو کل تصویر را با $\mathrm{PSNR} > 50\mathrm{\ dB}$ حفظ خواهد نمود. نتایج تجربی فصل چهارم (بخش ۴{-}۷) اثبات کرد که جای‌دهی ۲ بیتی \lr{LSB} فراداده فشرده‌شده \lr{zlib} صرفاً در پس‌زمینه کم‌برجستگی، موجب صیانت ۱۰۰ درصدی از بافت تشخیصی گردیده است (\textbf{$\mathrm{PSNR}_{\mathrm{ROI}} = \infty$} و $\mathrm{MSE}_{\mathrm{ROI}} = 0$). همچنین، متوسط شاخص کیفیت دیداری کل تصویر به \textbf{$\mathrm{PSNR}_{\mathrm{Stego}} = 53.54\mathrm{\ dB}$}، شاخص شباهت ساختاری به \textbf{$\mathrm{SSIM} = 0.9999$} و ظرفیت جای‌دهی به \textbf{$1.18\mathrm{\ bpp}$} رسید که عدم امکان تشخیص دیداری تغییرات توسط متخصصان رادیولوژی را تضمین می‌کند \ThesisCite{3}, \ThesisCite{8}, \ThesisCite{9}. آزمایش‌های پایداری (بخش ۴{-}۸) نیز رفتار سامانه را در شرایط بروز نویز و انسداد مشخص ساخت؛ اگرچه انتشار سراسری خطای ناشی از نگاشت‌های آشوبی در غیاب کدهای اصلاح خطا منجر به افت کیفیت در حملات شدید می‌گردد ($PSNR = 27.74\text{ dB}$ برای نویز ۱٪ و $17.78\text{ dB}$ برای انسداد ۱۰٪)، اما سازوکار احرازهویت بسته \lr{AES-GCM} در پرونده کمکی، وقوع هرگونه دستکاری را پیش از پردازش بالینی کشف و از تحویل تصویر نامعتبر جلوگیری می‌نماید. این دستاوردها فرضیه سوم پژوهش را نیز به صورت قطعی اثبات می‌نمایند \ThesisCite{5}, \ThesisCite{6}.\par
...
{-} \textbf{راهکار و دستاورد تجربی این پژوهش:} با بهینه‌سازی محاسباتی کدها و ارزیابی واقعی بر روی پلتفرم کم‌مصرف پردازش لبه \lr{NVIDIA Jetson TX2}، زمان اجرای کل سیستم برای یک تصویر پزشکی استاندارد به \textbf{۲.۸۵ ثانیه} رسید (تفکیک زمان اجرا: قطعه‌بندی \lr{U{-}Net} و کلید ۰.۸۲ ثانیه، سیستم ۵بعدی ۰.۴۵ ثانیه، زیگ‌زاگ و \lr{DNA} برابر ۰.۹۸ ثانیه، و \lr{zlib/Saliency} برابر ۰.۶۰ ثانیه). در بستر سرور/کولب، زمان اجرای زنده خط لوله برای تصویر \lr{DRIVE} (۵۸۴×۵۶۵) برابر با \textbf{۰.۵۸۴۶ ثانیه} ثبت شده است. این زمان اجرا قابلیت استقرار زمان‌واقعی سیستم را در شبکه \lr{IoMT} و سلامت همراه به اثبات رساند \ThesisCite{10}, \ThesisCite{11}.\par
...
سیستم پیشنهادی در شرایط کانال‌های ایده‌آل، قابلیت بازیابی را با کیفیت بالا نشان می‌دهد؛ اما به دلیل خاصیت انتشار سراسری نگاشت‌های آشوبی، اعمال نویز ۱٪ فلفل‌نمکی کیفیت بازسازی را به $PSNR = 27.74\text{ dB}$ و برش ۱۰٪ داده آن را به $17.78\text{ dB}$ کاهش می‌دهد. در پروتکل پیشنهادی، تگ احرازهویت \lr{AES-GCM} در پرونده کمکی (\texttt{recovery.msr})، بسته‌های دستکاری‌شده را پیش از پردازش بالینی رد می‌کند. علاوه بر این، زمان اجرای زنده خط لوله در سرور/کولب \textbf{۰.۵۸۴۶ ثانیه} و بنچمارک سخت‌افزار لبه \lr{NVIDIA Jetson TX2} در حدود \textbf{۲.۸۵ ثانیه} ثبت شده که استقرار عملیاتی را در تجهیزات \lr{IoMT} اثبات می‌نماید \ThesisCite{10}, \ThesisCite{11}.\par
...
سنجش زمان اجرای سیستم بر روی پلتفرم کم‌مصرف پردازش لبه \lr{NVIDIA Jetson TX2} نشان داد که زمان اجرای کل برای یک تصویر پزشکی استاندارد به \textbf{۲.۸۵ ثانیه} رسید (استنتاج \lr{U{-}Net}: ۰.۸۲ ثانیه، سیستم ۵بعدی: ۰.۴۵ ثانیه، زیگ‌زاگ و \lr{DNA}: ۰.۹۸ ثانیه، \lr{zlib/Saliency}: ۰.۶۰ ثانیه) \ThesisCite{10}, \ThesisCite{11}. در بستر سرور/کولب، زمان اجرای زنده خط لوله برای تصویر \lr{DRIVE} (۵۸۴×۵۶۵) برابر با \textbf{۰.۵۸۴۶ ثانیه} ثبت شده است. تحلیل تفکیکی زمان اجرا آشکار ساخت که استنتاج شبکه عصبی عمیق \lr{U{-}Net} جهت قطعه‌بندی \lr{ROI} سهمی معادل \textbf{۰.۸۲ ثانیه (۲۸.۸\SourceGlyph{066A} از زمان کل لبه)} را به خود اختصاص داده است \ThesisCite{1}, \ThesisCite{10}. این امر نشان می‌دهد که پیاده‌سازی مدل‌های عمیق بر روی تجهیزات کم‌مصرف پوشیدنی یا حسگرهای محدود \lr{IoMT} می‌تواند با چالش مصرف انرژی و گلوگاه استنتاج مواجه گردد.\par
...
تولید کلید هَش ۲۵۶ بیتی $K_{\mathrm{hash}}$ به صورت کاملاً پویا و بر پایه گشتاورهای آماری \lr{ROI} انجام می‌شود؛ اما برای رمزگشایی موفق در سمت گیرنده، پرونده کمکی \texttt{recovery.msr} باید از طریق کانال امن (\lr{PKI} یا \lr{Diffie-Hellman}) به گیرنده انتقال یابد. این پرونده، با استاندارد \lr{AES-GCM-256} رمزنگاری شده و تگ احرازهویت آن، بسته‌های دستکاری‌شده را پیش از پردازش بالینی رد می‌کند. وابستگی بازسازی بیت‌به‌بیت ($\mathrm{PSNR}_{\mathrm{ROI}} = \infty$) به این پرونده، اثبات نابینا بودن مطلق را باطل و جایگزین می‌کند \ThesisCite{1}, \ThesisCite{2}, \ThesisCite{4}.\par
...
۳. \textbf{طراحی پروتکل انتقال امن پرونده کمکی (\texorpdfstring{\lr{Sidecar Secure Transport Protocol}}{Sidecar Secure Transport Protocol}):}\par
به منظور رفع وابستگی به کانال‌های ایمن جداگانه، توسعه یک پروتکل یکپارچه انتقال همزمان تصویر استگو و پرونده \texttt{recovery.msr} با احرازهویت و رمزنگاری ساختاریافته \lr{AES-GCM-256} پیشنهاد می‌شود که با تأیید صحت تگ احرازهویت، بازیابی قطعی را بدون نیاز به زیرساخت \lr{PKI} جداگانه تضمین نماید \ThesisCite{2}.\par

۴. \textbf{بهره‌گیری از معماری‌های سبُک‌تر نظیر \lr{Vision Transformers} \lr{(ViT)} و \lr{MobileNetV4}:}\par
