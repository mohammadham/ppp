...
برای طراحی و پیاده‌سازی یک سیستم امنیتی جامع که محرمانگی تصویر تشخیصی پزشکی و صیانت از فراداده‌های هویتی بیمار را به صورت هم‌زمان تضمین نماید، شناخت عمیق مبانی نظری و تحلیل نقادانه پیشینه پژوهش امری ضروری است \ThesisCite{1}, \ThesisCite{2}, \ThesisCite{6}, \ThesisCite{8}, \ThesisCite{9}. در این فصل، سه محور اصلی حوزه امنیت داده‌های سلامت دیجیتال مورد بررسی و ریشه‌یابی علمی قرار می‌گیرند: (۱) اصول رمزنگاری تصویر مبتنی بر سیستم‌های غیرخطی آشوبی، سیستم‌های ابرآشوبی ۵ بعدی و تبدیل‌های ریاضی، (۲) نقش شبکه‌های عصبی عمیق (به‌ویژه معماری \lr{U{-}Net}) در قطعه‌بندی هوشمند، استخراج ویژگی‌های ناحیه حساس \lr{(ROI)} و تولید کلیدهای رمزی وابسته به محتوا، و (۳) تکنیک‌های پنهان‌نگاری کلاسیک و تطبیقی مبتنی بر برجستگی بصری \lr{(Saliency Detection)} جهت جای‌دهی امن فراداده‌ها بدون ایجاد اعوجاج در کیفیت بالینی \ThesisCite{1}, \ThesisCite{3}, \ThesisCite{9}. بررسی این محورهای سه‌گانه، شالوده نظری و روش‌شناختی لازم برای تحقق معماری هیبریدی پیشنهادی در این پایان‌نامه را فراهم می‌سازد که نمای کلی آن در شکل ۲{-}۱ ترسیم شده است.\par
...
این نگاشت غیرخطی با معادله دیفرانسیل گسسته زیر تعریف می‌شود \ThesisCite{6}:\par
...
معادلات دیفرانسیل حالت سیستم ابرآشوبی ۵ بعدی پیوسته به صورت فرمول‌بندی غیرخطی زیر بیان می‌گردند \ThesisCite{1}:\par
...
که در آن $(x,\allowbreak{} y,\allowbreak{} z,\allowbreak{} u,\allowbreak{} v)$ متغیرهای حالت سیستم در فضای پنج‌بعدی و پارامترهای کنترلی تثبیت‌شده در پیاده‌سازی محاسباتی به صورت $\alpha = 40.0$، $\beta = 8.0$، $\gamma = 40.0$، $\delta = 1.0$، $\epsilon = -0.5$، $\vartheta = -0.5$، $\rho = 25.5$ و $\kappa = 0.05$ تعریف شده‌اند. محاسبه توان‌های نمای لیاپانوف با الگوریتم تجزیه \lr{QR} در حالت ماندگار، مقادیر پنج نمای لیاپانوف را برابر با $\lambda_1 = 19.3889$، $\lambda_2 = 4.7545$، $\lambda_3 = 0.3866$، $\lambda_4 = -0.5982$ و $\lambda_5 = -44.4191$ نشان می‌دهد؛ وجود سه نمای لیاپانوف مثبت ($\lambda_1, \lambda_2, \lambda_3 > 0$) دلالت بر رفتار ابرآشوبی قدرتمند، گسترش تصادفی فاز در چند جهت متعامد و حساسیت بسیار بالا نسبت به شرایط اولیه دارد؛ ویژگی‌هایی که حد آرمانی مقاومت در برابر تحلیل‌های رمزنگاری تفاضلی را برآورده می‌سازند.\par
...
فرمول‌بندی لایه‌های کانولوشن، ادغام اتصالات پرشی و تابع زیان \lr{(Loss Function)} ترکیبی شبکه \lr{U{-}Net} به صورت زیر تعریف می‌گردند \ThesisCite{1}:\par
...
آنتروپی معیاری برای سنجش درجه تصادفی بودن خروجی رمزشده است و ضریب همبستگی میزان وابستگی مکانی پیکسل‌های همجوار را می‌سنجد \ThesisCite{1}, \ThesisCite{6}:\par
...
این دو معیار، میزان حساسیت تصویر رمزشده به تغییر یک پیکسلی در تصویر واضح ورودی را ارزیابی می‌کنند \ThesisCite{1}, \ThesisCite{6}:\par
...
برای ارزیابی عدم اعوجاج و میزان شفافیت تصویر پنهان‌نگاری‌شده نسبت به تصویر اصلی، از نسبت سیگنال به نویز بیشینه \lr{(PSNR)} و شاخص شباهت ساختاری \lr{(SSIM)} استفاده می‌شود \ThesisCite{3}, \ThesisCite{9}:\par
...
۱. بررسی مقاله \lr{Pourasad et al}. \lr{(2021)} {-} ترکیب نگاشت‌های آشوبی و حوزه تبدیل موجک \ThesisCite{6}:\par
...
۴. بررسی مقاله \lr{Alarood et al}. \lr{(2023)} {-} عمیق‌سازی رمزنگاری \lr{E{-}Health} با شبکه \lr{DNN} و نگاشت هنون \ThesisCite{2}:\par
...
۵. بررسی مقاله \lr{Husamuldeen et al}. \lr{(2025)} {-} تولید کلید پویا با \lr{EXIF Metadata} و شبکه‌های \lr{ResNet{-}18 / MobileNetV2} \ThesisCite{4}:\par
...
۶. بررسی مقاله \lr{Kumar et al}. \lr{(2025)} {-} رمزنگاری عمیق با \lr{DCGAN} و دامنه سیاره مجازی \lr{(VPD)} \ThesisCite{5}:\par
...
۷. بررسی مقاله \lr{Y\SourceGlyph{0131}ld\SourceGlyph{0131}r\SourceGlyph{0131}m} \lr{(2021)} {-} پنهان‌سازی صوت رمزنگاری‌شده در تصاویر \lr{CT} کووید{-}۱۹ \ThesisCite{9}:\par
...
۸. بررسی مقاله \lr{Alshamrani et al}. \lr{(2025)} {-} سامانه \lr{VisionStego AI} و ارزیابی تطبیقی \ThesisCite{8}:\par
...
۹. بررسی مقاله \lr{Issac et al}. \lr{(2025)} {-} پنهان‌نگاری عمیق با ماژول‌های \lr{SE} و \lr{Inception} بر روی سخت‌افزار لبه \ThesisCite{10}:\par
...
۱۰. بررسی مقاله \lr{Saidi et al}. \lr{(2024)} {-} پنهان‌نگاری با ظرفیت بالا مبتنی بر \lr{Mask{-}RCNN} و تبدیل \lr{DCT} \ThesisCite{3}:\par
...
% SOURCE Synthesis_Research_Gap_Chapter2.txt:6-6 [paragraph]
ماتریس مقایسه‌ای کارهای پیشین در جدول ۲{-}۱ ارائه شده است. لازم به ذکر است که مقادیر درج‌شده در این جدول، نقل‌quote مستقیم از متن مقالات مرجع منتشرشده بوده و کدهای این مقالات به دلیل عدم دسترسی به پیاده‌سازی کامل متن‌باز مؤلفان، بازاجرا نشده‌اند. همچنین زمان‌های اجرای قیدشده تابع پیکربندی‌های سخت‌افزاری، فرکانس پردازنده و محیط‌های برنامه‌نویسی گوناگون در مطالعات مبدأ بوده و به عنوان سنجه‌ای از پیچیدگی ذاتی الگوریتم‌ها و نه ارزیابی هم‌زمان در سخت‌افزار یکسان، گزارش شده‌اند.\par
...
اکثر سامانه‌های رمزشده آشوبی سنتی از کلیدهای ایستا یا وابسته به پارامترهای ثابت استفاده می‌کنند که سیستم را در برابر حملات متن‌واضح برگزیده \lr{(Chosen{-}Plaintext Attack)} و متن‌واضح معلوم \lr{(Known{-}Plaintext Attack)} بسیار آسیب‌پذیر می‌سازد \ThesisCite{2}, \ThesisCite{6}. در پژوهش‌هایی که کلید پویا پیشنهاد گردیده است (مانند استفاده از \lr{EXIF Metadata})، مکانیزم انتقال شرایط اولیه به گیرنده شفاف نبوده یا خودِ فراداده‌های هویتی در طول مسیر ارتباطی بدون محافظت رها شده‌اند \ThesisCite{4}. علاوه بر این، وابستگی کلید به تمام تصویر باعث می‌شود کوچک‌ترین نویز در پس‌زمینه تصویر واضح، مانع رمزگشایی صحیح گردد. این شکاف مستقیماً با «فرضیه اول پژوهش» (تولید کلید پویا بر پایه استخراج ویژگی‌های آماری \lr{ROI} شبکه \lr{U{-}Net} و تابع هش \lr{SHA{-}256}) پاسخ داده می‌شود. در راستای این فرضیه، سامانه پیشنهادی از یک سازوکار تبادل کمکی احرازهویت‌شده \lr{(Authenticated Recovery Sidecar)} مبتنی بر رمزنگاری متقارن احرازهویت‌شده \lr{(AEAD)} استفاده می‌کند تا بازگشایی قطعی و بیت‌به‌بیت تصویر اصلی در سمت گیرنده، مشروط به دریافت و اعتبارسنجی موفق پرونده کمکی رمزشده، محقق گردد.\par
...
روش‌های پنهان‌نگاری موجود (اعم از الگوریتم‌های مبتنی بر یادگیری عمیق مانند \lr{SE{-}Inception} و \lr{Mask{-}RCNN} یا روش‌های کلاسیک \lr{LSB/PVD})، فراداده‌های بیمار را بدون توجه دقیق به ارزش بالینی نواحی تصویر جای‌دهی می‌کنند \ThesisCite{3}, \ThesisCite{9}, \ThesisCite{10}. این امر منجر به ایجاد نویز دیداری، اعوجاج آناتومیک و افت کیفیت تشخیصی \lr{(PSNR \SourceGlyph{003C} 40 dB)} در ناحیه حساس تشخیصی \lr{(ROI)} می‌گردد. از سوی دیگر، روش‌هایی که کیفیت بالایی ارائه می‌دهند، فاقد ماژول رمزنگاری آشوبی برای خودِ تصویر تشخیصی هستند \ThesisCite{8}. این شکاف مستقیماً با «فرضیه سوم پژوهش» (پنهان‌نگاری فراداده فشرده‌سازی‌شده بیمار بر پایه نقشه برجستگی بصری \lr{Saliency Detection} در پس‌زمینه بی‌اهمیت تصویر با \lr{PSNR \SourceGlyph{003E} 50 dB} و \lr{SSIM} \SourceGlyph{007E} \lr{1}) برطرف می‌شود.\par
